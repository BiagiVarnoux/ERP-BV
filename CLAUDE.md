...
Hay **dos formularios** y el lector soporta ambos: la **DIM R-505** completa y la **DIMS R-510** simplificada (despacho de menor cuantía). Difieren en dónde está el CIF en bolivianos — `F10. Total valor CIF aduana (BOB)` en la R-505, `E7. Valor CIF total (Bs)` en la R-510 — y en cuántas columnas tiene la tabla de tributos (por eso el GA y el IVA se toman como el ÚLTIMO número de su fila, no por posición fija). En la R-510 los rótulos y los valores van en líneas separadas y se emparejan por posición. En una DIMS el **declarante puede ser el propio importador** (B1 = B2): eso es correcto, no un error de lectura. Además `numero_declaracion` guarda el N° de la declaración (campo A1) en su propia columna.

**Control cruzado obligatorio**: `baseCoherenteConIva()` verifica que `CIF + GA` se parezca a `iva / 0,1494`. Si no cuadra, el formulario avisa. Esto cazó un caso real donde el lector tomó el GA (285) como si fuera el CIF y la base quedó en 285 en vez de 12.479,31.
...
⚠️ **Edge Function y cliente NO se despliegan a la vez.** La función se despliega al instante; el cliente llega cuando el usuario recarga. Cambiar una clave del JSON de respuesta rompe a todo cliente que siga abierto — pasó al renombrar `razon_social` a `razon_social_emisor`: el proveedor quedaba vacío y el resto del formulario se llenaba bien (el síntoma delata qué claves cambiaron). Por eso `ai-factura` **resuelve también `razon_social`/`nit`** antes de responder, además de las claves nuevas. Regla: al cambiar el contrato de una Edge Function, añadir claves y mantener las viejas; nunca renombrar en un solo paso.

-- fix: create_sale RPC filtraba inventory_movements e inventory_lots por user_id
-- en lugar de company_id, causando error "Stock insuficiente" para usuarios no-owner.
-- Cambio: AND user_id = v_user_id → AND company_id = v_company_id en ambas secciones
-- (CPP y FIFO). El user_id sigue usándose solo para registrar quién creó el movimiento.

-- ================================================================
-- Migration: Security fixes (SOC2/NIST audit 2026-06-06)
-- Fixes:
--   H1: RLS hardcodeada en receivables/payables/debt_payments
--   H2: redeem_invitation_code sin validación de auth.uid()
--   H3: create_sale sin validación de membresía de company_id
--   H4: audit_log sin políticas RESTRICTIVE de DELETE/UPDATE
-- ================================================================

-- ================================================================
-- H1: Reemplazar RLS hardcodeada con scoping real por membresía
-- ================================================================

-- receivables
DROP POLICY IF EXISTS "company_isolation" ON public.receivables;

CREATE POLICY "company_member_read" ON public.receivables
  FOR SELECT TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_insert" ON public.receivables
  FOR INSERT TO authenticated
  WITH CHECK (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_update" ON public.receivables
...
CREATE POLICY "company_member_delete" ON public.receivables
...
-- payables
DROP POLICY IF EXISTS "company_isolation" ON public.payables;

CREATE POLICY "company_member_read" ON public.payables
  FOR SELECT TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_insert" ON public.payables
  FOR INSERT TO authenticated
  WITH CHECK (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_update" ON public.payables
  FOR UPDATE TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ))
...
CREATE POLICY "company_member_delete" ON public.payables
  FOR DELETE TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));
...
-- debt_payments
DROP POLICY IF EXISTS "company_isolation" ON public.debt_payments;

CREATE POLICY "company_member_read" ON public.debt_payments
  FOR SELECT TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_insert" ON public.debt_payments
  FOR INSERT TO authenticated
  WITH CHECK (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_update" ON public.debt_payments
  FOR UPDATE TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));

CREATE POLICY "company_member_delete" ON public.debt_payments
  FOR DELETE TO authenticated
  USING (company_id IN (
    SELECT company_id FROM public.company_members WHERE user_id = auth.uid()
  ));
...
-- ================================================================
-- H2: redeem_invitation_code — agregar validación de auth.uid()
-- ================================================================
...
DECLARE
  ...
  _company_id uuid;
BEGIN
  -- SECURITY FIX (H2): Prevent redeeming on behalf of another user
...
-- ================================================================
-- H3: create_sale — validar membresía del company_id recibido
--     Cuerpo idéntico al de 20260601000002, solo se agregan las
--     líneas de validación justo después del check de auth.uid().
-- ================================================================
...
BEGIN
  ...
  -- SECURITY FIX (H3): Validate that the caller belongs to the target company
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(payload->'items') LOOP
    ...
    IF v_metodo = 'CPP' THEN
      ...
      INSERT INTO public.inventory_movements (
        ...
        'CPP', COALESCE(v_glosa, v_numero), v_entry_id, v_fecha, v_user_id, v_company_id
      );
    ...
    ELSIF v_metodo = 'FIFO' THEN
      ...
      LOOP
        ...
        INSERT INTO public.inventory_movements (
          ...
          v_mov_id, v_product_id, v_lot.id, 'SALIDA', v_take,
          v_lot.costo_unitario, round((v_take * v_lot.costo_unitario)::numeric, 2),
          'FIFO', COALESCE(v_glosa, v_numero), v_entry_id, v_fecha, v_user_id, v_company_id
        );
    ...
    INSERT INTO public.sale_items (
      sale_id, product_id, product_nombre, product_codigo,
      cuenta_inventario_id, metodo_valuacion, cantidad,
      precio_unitario_neto, subtotal_neto, costo_unitario,
      costo_total, margen_bruto, inventory_movement_id
    ) VALUES (
      v_sale_id, v_product_id, v_item->>'product_nombre', v_item->>'product_codigo',
      v_cuenta_inv, v_metodo, v_cantidad,
      v_precio_u, v_subtotal, v_costo_u,
      v_costo_t, round((v_subtotal - v_costo_t)::numeric, 2), v_mov_id
    );
  ...
  IF v_tipo_pago IN ('cxc', 'cxc_licitaciones') THEN
    INSERT INTO public.receivables (
      company_id, user_id, customer_id, sale_id,
      numero_documento, fecha_emision, fecha_vencimiento,
      monto_original, monto_pendiente, moneda, estado
    ) VALUES (
      v_company_id, v_user_id,
      NULLIF(payload->>'customer_id', '')::uuid, v_sale_id,
      v_numero, v_fecha, NULL,
      v_total_cobrado, v_total_cobrado, 'BOB', 'open'
    );
  END IF;
...
-- ================================================================
-- H4: audit_log — políticas RESTRICTIVE que bloquean DELETE/UPDATE
--     Garantizan no-repudio / inmutabilidad del registro de auditoría
-- ================================================================

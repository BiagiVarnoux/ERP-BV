-- FIX de pérdida de datos: build_company_backup (backup del servidor) NO incluía
-- product_fotos, company_sale_account_config ni product_publicaciones, PERO el
-- backup/restore del cliente SÍ las borra y repone. Resultado: al restaurar un
-- snapshot automático del servidor (que no traía esas tablas), la fase de borrado
-- del cliente las vaciaba y no había nada que reponer.
--   · product_fotos → se perdían las fotos del catálogo (los archivos quedan en
--     Storage, pero sin metadatos la app no los ve).
--   · company_sale_account_config → métodos de pago de ventas.
--   · product_publicaciones → qué productos publicó cada vendedor.
--
-- Fix: incluir las 3 tablas (todas con company_id directo) en el backup del
-- servidor, para que cliente y servidor queden sincronizados (regla de los 3
-- lugares). Se sube la versión a 3.2.

-- create_sale ahora acredita la cuenta de inventario de CADA lote consumido, no la
-- del producto. Antes, al vender se acreditaba siempre products.cuenta_inventario_id
-- (vía el sale_item). Tras habilitar transferencias entre cuentas, un mismo producto
-- puede tener lotes en cuentas distintas (p.ej. parte en A.4.3 Licitaciones y parte
-- en A.4.6 Otros); el COGS debe descargar el inventario de la cuenta real de cada lote.
--   · FIFO → se acumula v_inv_totals por la cuenta del lote (fallback: la del producto)
--   · CPP  → se mantiene el comportamiento anterior (cuenta del producto)

...
function rowToLicitacion(row: Record<string, unknown>): Licitacion {
  return {
    ...
    flete_cif_pct:         row.flete_cif_pct != null ? Number(row.flete_cif_pct) : undefined,
    ...
    garantia_licitacion:      Number(row.garantia_licitacion) || 0,
    pasaje_licitacion:        Number(row.pasaje_licitacion) || 0,
    envio_licitacion:         Number(row.envio_licitacion) || 0,
    otros_costos_licitacion:  Number(row.otros_costos_licitacion) || 0,
...
function rowToProducto(row: Record<string, unknown>): LicitacionProducto {
  return {
    ...
    origen:          (row.origen as 'importado' | 'local') || 'importado',
    precio_local:    row.precio_local != null ? Number(row.precio_local) : undefined,
    tiene_factura:   Boolean(row.tiene_factura),
...
export const LicitacionStorage = {
  ...
  async upsertProductos(productos: LicitacionProducto[]): Promise<void> {
    ...
    const rows = safe.map(p => ({
      ...
      flete_cif_pct:    p.flete_cif_pct ?? null,
      ...
      ga_pct:              p.ga_pct,
      flete_manual:        p.flete_manual ?? null,
      usa_flete_manual:    p.usa_flete_manual ?? false,
      flete_manual_es_total: p.flete_manual_es_total ?? false,
      manipuleo_manual:    p.manipuleo_manual ?? null,
      usa_manipuleo_manual: p.usa_manipuleo_manual ?? false,
      manipuleo_manual_es_total: p.manipuleo_manual_es_total ?? false,
      ga_manual:           p.ga_manual ?? null,
      usa_ga_manual:       p.usa_ga_manual,
      ga_manual_es_total:  p.ga_manual_es_total ?? false,
      iva_aduana_manual:   p.iva_aduana_manual ?? null,
      usa_iva_manual:      p.usa_iva_manual,
      iva_manual_es_total: p.iva_manual_es_total ?? false,
